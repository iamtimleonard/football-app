...
async function getSeason(year, team) {
  let { data } = await axios
    .get(`http://localhost:3000`, { params: { year, team } })
    .catch(function (err) {
      printError();
    });
  return data;
...
const createCell = (stat, data, home) => {
  ...
  const { away_team, home_team } = data;
...
const printError = () => {
  document.querySelector(".season-record").textContent =
    "No data available: Please enter a valid school and year";
  return;
};
...
const gameMatchup = (data, homeGame) => {
  ...
  //win/loss heading
  let resultsContent =
    (homeGame && home_points > away_points) ||
    (!homeGame && away_points > home_points)
      ? `${searchedTeam} Victory`
      : `${searchedTeam} Loss`;
  ...
  for (let i = 1; i < 5; i++) {
    ...
    createTextElement(
      "td",
      `${
        home_line_scores[i - 1] == undefined
          ? "No data"
          : home_line_scores[i - 1]
      }`,
      row
    );
    createTextElement(
      "td",
      `${
        away_line_scores[i - 1] == undefined
          ? "No data"
          : away_line_scores[i - 1]
      }`,
      row
    );
  }
  ...
  searchMatchup.addEventListener("click", async function () {
    const { data } = await axios.get("http://localhost:3000/matchup", {
      params: { searchedTeam, opponent },
    });
...
const makeMatchupHistory = (data) => {
  removeItems(".summary");
  const { team1, team2, team1Wins, team2Wins, ties, games } = data;
  const detailsSection = document.querySelector(".details");
  ...
  createTextElement(
    "h3",
    `${team1} vs. ${team2} Matchup history`,
    matchupHistory
  );
  createTextElement(
    "p",
    `${team1} has ${team1Wins} wins, ${team2Wins} losses, and ${ties} ties against ${team2}`,
    matchupHistory
  );
  ...
  for (let i = games.length > 5 ? games.length - 5 : 0; i < games.length; i++) {
    const { awayScore, homeScore, season, winner } = games[i];
    createTextElement(
      "p",
      `${season}: ${homeScore} - ${awayScore}, ${winner} win`,
      matchupHistory
    );
  }
};
...
const populateSeason = (season) => {
  ...
  for (let game of season) {
    const { week, away_team, home_team, away_points, home_points } = game;
    const homeGame =
      home_team.toUpperCase() == inputContent.school.toUpperCase()
        ? true
        : false;
    const win =
      (homeGame && home_points > away_points) ||
    ...
    const row = createRow(game, homeGame, win, [
      week,
      home_team,
      away_team,
      home_points,
  ...
  recordHeading.textContent = `${inputContent.school.toUpperCase()} ${
    inputContent.year
  } SEASON: ${wins} - ${losses}`;
};
